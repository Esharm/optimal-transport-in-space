...
CONFIG = {
    ...
    # Input/output paths
    # --------------------------------------------------------
    "ground_truth_dir": "aart_test_results/ground_truth_average",
    "reconstruction_dir": "aart_test_results/reconstructed_frames_gray",
    "output_dir": "aart_test_results/evaluation",
    ...
    # --------------------------------------------------------
    "normalization": "minmax",  # minmax, flux, zscore, none
    "total_flux": 1.0,
    "fov_uas": 160.0,
    ...
    "align_reconstruction": False,
    "max_shift_pixels": 0,
    "allow_ehtim_shift": False,
...
VALID_UV_SUFFIXES = {".csv", ".txt", ".dat", ".npy", ".npz", ".uvfits"}
EPS = 1e-12
...
def make_visibility_pair_loader(uv_config, n_frames, verbose=True):
    ...
    if not use_dirs and not use_files:
        if verbose:
            print("WARNING: uv_data.enabled=True, but no u-v folders or files were provided. Using image-based Fourier metrics.")
        return lambda frame_index: (None, None, None), False
...
def evaluate_folders(config):
    ...
    # ========================================================
    # Sequence-level temporal variance-map metric
    # ========================================================
    ...
    if verbose:
        ...
        if save_intermediate_csvs:
            ...
            if not temporal_df.empty:
                print(f"Saved temporal variance CSV to:      {output_dir / 'temporal_variance_metrics.csv'}")
            if not uv_df.empty:
        ...
        if not temporal_df.empty:
            print("=" * 60)
            print("Temporal variance-map metrics:")
            print(temporal_df)
...
def apply_cli_overrides(config):
    ...
    for key in ["ground_truth_uv_dir", "reconstruction_uv_dir", "ground_truth_uv_file", "reconstruction_uv_file"]:
        val = getattr(args, key)
        if val is not None:
