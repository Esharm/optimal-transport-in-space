...
$$
D_k(u_k)=\frac{1}{2}|A_k u_k-y_k|_2^2,
$$
...
$$
\rho^+*{k,0}=p_k,\quad \rho^+*{k,T-1}=p_{k+1},
$$

$$
\rho^-*{k,0}=n_k,\quad \rho^-*{k,T-1}=n_{k+1}.
$$
...
\beta\sum_{k=0}^{K-2}
\left[
\mathcal{A}_\gamma(\rho^+_k,m^+_k,s^+*k)
+\mathcal{A}*\gamma(\rho^-_k,m^-_k,s^-_k)
\right].
$$
