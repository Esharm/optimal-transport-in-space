"""External static initialization followed by positive-residual BB/ADMM OT.
...
Workflow:
    ...
    4. Run ADMM with the data term on u_k, but balanced BB transport on

           h_k = max(u_k - a, 0).

This is not unbalanced OT. It is balanced BB OT applied to the positive
dynamic residual so that the temporal regularizer acts on flare-like excess
brightness instead of the full mostly-static ring.
"""
...
from admm import PositiveResidualOTADMM
...
class Config(LoaderConfig):
    ...
    output_root: Path = Path("main6_residual_ot_results")
...
def run_positive_residual_ot(u_init, data_terms, background, cfg):
    ...
    print(
        "Starting positive-residual balanced BB/ADMM OT "
...
def save_comparison_video(gt, static, ot, root: Path, fps: int):
    ...
    output_path = root / "comparison_gt_static_residual_ot.mp4"
    first = make_labeled_panel([gt[0], static[0], ot[0]], ["GT", "static", "residual OT"])
    ...
    for k in range(1, min(len(gt), len(static), len(ot))):
        writer.write(make_labeled_panel([gt[k], static[k], ot[k]], ["GT", "static", "residual OT"]))
    writer.release()
    return output_path
...
def residual_diagnostics(label, frames, background):
    residual = np.maximum(np.asarray(frames) - background[None, :, :], 0.0)
    masses = residual.sum(axis=(1, 2))
    rows = sequence_diagnostics(f"{label}_positive_residual", residual)
    rows.update({
        f"{label}_positive_residual_mass_min": float(masses.min()),
        f"{label}_positive_residual_mass_max": float(masses.max()),
        f"{label}_positive_residual_mass_mean": float(masses.mean()),
        f"{label}_positive_residual_mass_relative_range": float(
            (masses.max() - masses.min()) / (masses.mean() + 1e-12)
        ),
    })
    return rows
...
def main():
    ...
    print("=" * 100)
    print("MAIN6.PY: EXTERNAL STATIC INIT -> POSITIVE-RESIDUAL BB/ADMM OT")
    print("Outputs are intentionally minimal: results.npz and one comparison video.")
    print("=" * 100)
    print("Residual model: h_k = max(u_k - background, 0); balanced BB acts on h_k.")
    ...
    print("\n" + "=" * 100)
    print("2. Positive-residual balanced BB/ADMM OT refinement")
    print("=" * 100)
    u_ot, ot_history, ot_elapsed, ot_converged = run_positive_residual_ot(
        u_static,
        data_terms,
        background,
        cfg,
    ...
    ot_summary = summary_row(
        "positive_residual_balanced_bb_admm_ot",
        u_ot,
        data_terms,
        background,
        cfg,
