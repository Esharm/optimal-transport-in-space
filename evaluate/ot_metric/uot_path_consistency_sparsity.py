...
Several reconstructions in one ordinary run:

    python uot_path_consistency_sparsity.py \
        --reference../../blackhole_sim/data/aart_frames \
        --sequence static=/path/to/static \
        --sequence static+UOT=/path/to/static_uot \
        --sequence StarWarps=/path/to/starwarps \
        --sequence StarWarps+UOT=/path/to/starwarps_uot \
        --lags 2 3 4 5 \
        --json results.json --csv results.csv --plot results.png

Sparsity sweep for levels 8, 12, 21, and 34:

    python uot_path_consistency_sparsity.py \
        --reference ../../blackhole_sim/data/aart_frames \
        --method-template "static=../../results/static_reconstruction_results/reconstructed_frames_gray_{sparsity}" \
        --method-template "static+UOT=../../results/optimal_transport_results/ot_uot_static_{sparsity}/frames" \
        --method-template "StarWarps=../../results/starwarps_results/{sparsity}_telescopes/frames_{sparsity}_png" \
        --method-template "StarWarps+UOT=../../results/optimal_transport_results/ot_uot_SW_{sparsity}/frames" \
        --sparsity-levels 8 12 21 34 \
        --lags 2 3 4 5 \
        --output-dir otpc_by_sparsity

The sparsity sweep evaluates the reference once, prints a separate table for
each sparsity level, writes one PNG/CSV/JSON per level, and writes combined
long-form CSV, wide-form CSV, and JSON summaries. Quote template arguments in
the shell so that ``{sparsity}`` is not interpreted by shell brace expansion.
...
__version__ = "3.0.0-sparsity-batch"
...
def format_sparsity_template(template: str, sparsity: int) -> str:
    """Expand a candidate path template for one sparsity level.

    Both ``{sparsity}`` and ``{level}`` are accepted as placeholders.  A
    placeholder is required in batch mode so that accidental reuse of the same
    sequence at every sparsity level is caught immediately.
    """
    if "{sparsity}" not in template and "{level}" not in template:
        raise ValueError(
            "Each --method-template path must contain {sparsity} or {level}; "
            f"got {template!r}."
        )
    try:
        return template.format(sparsity=sparsity, level=sparsity)
    except (KeyError, IndexError, ValueError) as exc:
        raise ValueError(f"Invalid sparsity path template {template!r}: {exc}") from exc
...
def reference_intensity_scale(reference: np.ndarray) -> float:
    masses = reference.reshape(reference.shape[0], -1).sum(axis=1)
    scale = float(np.mean(masses))
    if not np.isfinite(scale) or scale <= 0:
        raise ValueError("Reference sequence has nonpositive mean total brightness.")
    return scale
...
def validate_frame_counts(
    ...
    n_frames = reference.shape[0]
    max_lag = max(lags)
    if n_frames <= max_lag:
        raise ValueError(
            f"Need at least {max_lag + 1} frames for lag {max_lag}; "
            f"reference has {n_frames}."
        )
    for label, frames in candidates.items():
        if frames.shape[0] != n_frames:
            raise ValueError(
                f"Sequence {label!r} has {frames.shape[0]} frames; "
                f"reference has {n_frames}."
            )
...
def write_json(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        "metric": "optimal_transport_path_consistency",
        "definition": {
            "window_error": (
                "source-weighted relative L1 difference between direct and "
                "composed UOT transfer kernels"
            ),
    ...
    if sparsity is not None:
        payload["sparsity_level"] = sparsity
...
def make_plot(
    ...
    lags: Sequence[int],
    *,
    title: str = "UOT path-consistency fidelity",
    dpi: int = 200,
) -> None:
    try:
    ...
    for comparison in comparisons:
        ...
        ax.plot(lags, values, marker="o", linewidth=2, label=comparison.label)
    ax.set_xlabel("Lag $\\ell$ (frames)")
    ax.set_ylabel("OTPC value $G_\\ell$ (difference from GT)")
    ax.set_title(title)
...
def print_report(
    ...
    *,
    ...
    headers, rows = _report_table_rows(comparisons, lags)
...
def write_batch_long_csv(
    path: Path,
    reference: SequenceConsistency,
    results: Mapping[int, Sequence[CandidateComparison]],
) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        ...
        for sparsity in sorted(results):
            for comparison in results[sparsity]:
                for lag in sorted(comparison.g_by_lag):
                    writer.writerow(
                        [
                            sparsity,
    ...
    with path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                "sparsity",
                "sequence",
                *[f"G_{lag}" for lag in lags],
                *[f"E_{lag}" for lag in lags],
            ]
        )
        for sparsity in sorted(results):
            for comparison in results[sparsity]:
                writer.writerow(
                    [
                        sparsity,
                        comparison.label,
                        *[comparison.g_by_lag[lag] for lag in lags],
                        *[comparison.raw_mean_by_lag[lag] for lag in lags],
                    ]
                )
...
def write_batch_json(
    ...
    reference_scale: float,
    results: Mapping[int, Sequence[CandidateComparison]],
    method_templates: Sequence[tuple[str, str]],
) -> None:
    payload = {
        "metric": "optimal_transport_path_consistency",
        "mode": "sparsity_batch",
        ...
        "reference_intensity_scale": reference_scale,
        "reference": asdict(reference),
        "method_templates": {label: template for label, template in method_templates},
        "sparsity_results": {
            str(sparsity): [asdict(comparison) for comparison in comparisons]
            for sparsity, comparisons in sorted(results.items())
        },
    }
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def evaluate_candidates(
    candidate_frames: Mapping[str, np.ndarray],
    reference_result: SequenceConsistency,
    config: EvaluatorConfig,
    *,
    verbose: bool,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compute multiscale UOT/OTPC gaps G_l for one reference and either "
            "ordinary candidate sequences or a sparsity sweep."
        ),
    ...
    parser.add_argument(
        ...
        metavar="LABEL=PATH",
        help=(
            "Single-run candidate sequence; repeat for multiple candidates. "
            "Do not combine with --method-template."
        ),
    )
    parser.add_argument(
        "--method-template",
        ...
        help=(
            "Batch-sparsity candidate path template. The path must contain "
            "{sparsity} or {level}. Repeat once per method, e.g. static, "
            "static+UOT, StarWarps, and StarWarps+UOT."
        ),
    )
    parser.add_argument(
        "--sparsity-levels",
        ...
        nargs="+",
        default=[8, 12, 21, 34],
        help="Sparsity levels evaluated in --method-template batch mode.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("otpc_sparsity_results"),
        help=(
            "Batch-mode output directory. Each sparsity gets a PNG, CSV, and JSON; "
            "combined long/wide CSV and JSON files are also written here."
        ),
    )
    parser.add_argument("--plot-dpi", type=int, default=200, help="PNG plot resolution.")
    ...
    parser.add_argument(
        "--entropic-reg", type=float, default=0.03,
        help="Entropic/KL coupling regularization epsilon."
    )
    parser.add_argument(
        "--marginal-reg", type=float, default=1.0,
        help="KL marginal-relaxation weight tau. Increase to transport more mass."
    )
    ...
    parser.add_argument("--json", type=Path, help="Single-run JSON output path.")
    parser.add_argument("--csv", type=Path, help="Single-run CSV summary path.")
    parser.add_argument("--plot", type=Path, help="Single-run G_l line-plot output path.")
...
def print_evaluator_summary(config: EvaluatorConfig) -> None:
...
def run_single_mode(args: argparse.Namespace, config: EvaluatorConfig) -> int:
    ...
    validate_frame_counts(reference_frames, candidate_frames, config.lags)
    reference_frames, candidate_frames, scale = normalize_sequences(
        reference_frames, candidate_frames
    )
    print_evaluator_summary(config)
    ...
    comparisons = evaluate_candidates(
        candidate_frames,
        reference_result,
        config,
        verbose=not args.quiet,
    ...
    if args.json:
        write_json(args.json, config, reference_result, comparisons, scale)
    ...
    if args.plot:
        make_plot(args.plot, comparisons, config.lags, dpi=args.plot_dpi)
        print(f"Wrote plot: {args.plot}", file=sys.stderr)
    return 0


def run_sparsity_batch(args: argparse.Namespace, config: EvaluatorConfig) -> int:
    assert args.method_template
    labels = [label for label, _ in args.method_template]
    if len(set(labels)) != len(labels):
        raise ValueError("Method-template labels must be unique.")
    sparsity_levels = sorted(set(args.sparsity_levels))
    if not sparsity_levels or any(level <= 0 for level in sparsity_levels):
        raise ValueError("All sparsity levels must be positive integers.")

    print(f"Loading reference once: {args.reference}", file=sys.stderr)
    reference_raw = load_sequence(args.reference, n=config.n)
    scale = reference_intensity_scale(reference_raw)
    reference_frames = reference_raw / scale
    validate_frame_counts(reference_frames, {}, config.lags)
    print_evaluator_summary(config)
    ...
    all_results: dict[int, list[CandidateComparison]] = {}

    for sparsity in sparsity_levels:
        print(f"\nLoading sparsity level {sparsity}", file=sys.stderr)
        candidates: dict[str, np.ndarray] = {}
        resolved_specs: dict[str, str] = {}
        for label, template in args.method_template:
            spec = format_sparsity_template(template, sparsity)
            resolved_specs[label] = spec
            print(f"  {label}: {spec}", file=sys.stderr)
            candidates[label] = load_sequence(spec, n=config.n)

        validate_frame_counts(reference_raw, candidates, config.lags)
        normalized_candidates = {label: frames / scale for label, frames in candidates.items()}
        comparisons = evaluate_candidates(
        ...
        all_results[sparsity] = comparisons

        print_report(
            reference_result,
            comparisons,
            config.lags,
            heading=f"OTPC RESULTS — SPARSITY {sparsity}",
        )

        stem = f"otpc_sparsity_{sparsity}"
        ...
        make_plot(
            ...
            comparisons,
            config.lags,
            title=f"{sparsity} telescopes OTPC per lag",
            dpi=args.plot_dpi,
        )
        write_csv(csv_path, reference_result, comparisons, sparsity=sparsity)
        write_json(
            json_path,
            config,
            reference_result,
        ...
        # Include concrete resolved inputs in the per-level JSON.
        payload = json.loads(json_path.read_text(encoding="utf-8"))
        payload["resolved_candidate_paths"] = resolved_specs
        json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")
        print(f"Wrote PNG:  {plot_path}", file=sys.stderr)
        print(f"Wrote CSV:  {csv_path}", file=sys.stderr)
        print(f"Wrote JSON: {json_path}", file=sys.stderr)

    write_batch_long_csv(
        args.output_dir / "otpc_all_sparsities_long.csv",
        reference_result,
        all_results,
    )
    write_batch_wide_csv(
        args.output_dir / "otpc_all_sparsities_wide.csv",
        all_results,
        config.lags,
    )
    write_batch_json(
        args.output_dir / "otpc_all_sparsities.json",
        config,
        reference_result,
        scale,
        all_results,
        args.method_template,
    )
    print(f"\nCombined outputs written to: {args.output_dir}", file=sys.stderr)
    return 0
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    has_single = bool(args.sequence)
    has_batch = bool(args.method_template)
    if has_single == has_batch:
        raise ValueError(
            "Choose exactly one mode: provide one or more --sequence arguments, "
            "or provide one or more --method-template arguments for a sparsity sweep."
        )
    if has_batch and (args.json or args.csv or args.plot):
        raise ValueError(
            "--json, --csv, and --plot are single-run options. In sparsity batch mode, "
            "use --output-dir; all per-level and combined outputs are generated automatically."
        )

    if has_batch:
        return run_sparsity_batch(args, config)
